...
export default async function AdminProducts({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; low?: string }> }) {
  ...
  return (
    <AdminPage title="Produits" sub={`${rows.length} affichés`} action={<div className="flex gap-2"><a href="/api/admin/export/products" className={abtnGhost}><DownloadIcon size={14} /> CSV</a><Link href="/admin/produits/nouveau" className={abtn}>Nouveau produit</Link></div>}>
