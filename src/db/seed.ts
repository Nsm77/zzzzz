...
async function main() {
  ...
  const [admin, support, customer] = await db.insert(users).values([
    { email: "admin@cleopatre.tn", passwordHash: await hash("Admin123!"), firstName: "Nour", lastName: "Ben Salah", role: "admin", phone: "71430500" },
    { email: "support@cleopatre.tn", passwordHash: await hash("Support123!"), firstName: "Sami", lastName: "Trabelsi", role: "support", phone: "71430501" },
    { email: "client@cleopatre.tn", passwordHash: await hash("Client123!"), firstName: "Ines", lastName: "Mansour", role: "customer", phone: "22345678", loyaltyPoints: 42 },
  ]).returning();
  ...
  for (const [name, brand, universe, cat, price, compare, vol, ks, short, opts] of P) {
    ...
    const [p] = await db.insert(products).values({
      ...
      image: imgFor[universe], images: [imgFor[universe]], volume: vol, status: "active", isFeatured: !!opts?.featured, isNew: !!opts?.isNew || n % 9 === 0,
  ...
  console.log(`✓ Seed complete — ${productIds.length} products. Admin: admin@cleopatre.tn / Admin123! · Client: client@cleopatre.tn / Client123! · Support: ${support.email} / Support123!`);
  await pool.end();
}
