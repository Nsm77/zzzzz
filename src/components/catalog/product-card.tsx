    ...
    <motion.article whileHover={reduce ? undefined : { y: -4 }} transition={{ duration: 0.5, ease: EASE_LUXE }} className="group relative flex h-full flex-col">
      <div className="relative aspect-[4/5] overflow-hidden bg-stone">
        <Link href={`/produit/${p.slug}`} aria-label={p.name} tabIndex={-1} className="absolute inset-0">
          {p.image && (
            <Image
              ...
              className={cn("object-cover transition-transform duration-[1400ms] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.05]", out && "opacity-55 grayscale-[35%]")}
            />
          )}
        </Link>
        {out && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-ink/10">
            <span className="border border-paper bg-paper/90 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.22em] text-ink">Épuisé</span>
          </div>
        )}
        ...
        <button
          ...
          className={cn("absolute right-2 top-2 z-10 flex h-11 w-11 items-center justify-center transition-colors duration-300", w ? "text-champagne-2" : "text-ink/60 hover:text-ink")}
        >
